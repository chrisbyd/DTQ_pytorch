...
class CrossFormerBlock(nn.Module):
    ...
    def forward(self, x):
        print("The input dimension for the cross former block {}".format(x.shape))
        ...
        print('The input to the cross former self attention in the block {}'.format(x.shape))
        ...
        x = x.view(B, H * W, C)
        print('The output from the cross former atttention block {}'.format(x.shape))
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=[4], in_chans=3, embed_dim=96, norm_layer=None):
        ...
        for i, ps in enumerate(patch_size):
            ...
            print("The {}th projection has dim {} and kernel_size {}, stride {}, padding is {}".format(i, dim, ps, stride, padding))
    ...
    def forward(self, x):
        print("The input shape for input image,",x.shape)
        B, C, H, W = x.shape
        ...
        for i in range(len(self.projs)):
            ...
            print("The {} th projection has shape {}".format(i, tx.shape))
...
class CrossFormer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=[4], in_chans=3,
        ...
        self.head = nn.Linear(self.num_features, 1000) if 1000 > 0 else nn.Identity()
